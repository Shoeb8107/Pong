...
use crate::paddle::{Paddle, Direction};
...
impl Game {
    ...
    pub fn draw(&self, con: &Context, g: &mut G2d, cache: &mut Glyphs) {
        ...
        draw_rectangle(BORDER_COLOR, (self.width - 1) as f64, MARGIN_TOP, 1, self.height, con, g);

        draw_text(std::format!("SCORE: {}", self.score).as_str(), 5.0, con, g, cache);

        if self.game_over {
            draw_rectangle(GAME_OVER_COLOR, 0.0, MARGIN_TOP, self.width, self.height, con, g);
        }
    }
