use piston_window::{Context, G2d, Glyphs, Transformed, rectangle, text, types::Color};
...
pub fn draw_rectangle(color: Color, x: f64, y: f64, width: i32, height: i32, con: &Context, g: &mut G2d){
    let x = to_coord(x);
    let y = to_coord(y);
